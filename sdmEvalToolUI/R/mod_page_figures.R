...
#' test_page_static()
#' test_page_static(deployment_id = "deployment2")
#' test_page_static(user_id = "testuser")

test_page_static <- function(...) {
  test_page("mod_page_static", ...)
}
...
#' @export
#' @examples
#' mod_page_static_ui("id", "title")

mod_page_static_ui <- function(
  id,
  title,
  review_width = NULL
) {
  nav_panel(
    ...
    sdm_layout_sidebar(
      ...
      mainPanel(
        ...
        mod_comp_static_ui(NS(id, "static"), header = "Extra figures"),
...
mod_page_static_server <- function(id = "static", ...) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    mod_utils_evaluations_server(
      "eval",
      component_id = c("static"), # component types included
    ...
    mod_comp_static_server("static", model_id, species_id)
    # mod_comp_model_fit_server('fit', model_id, species_id)
